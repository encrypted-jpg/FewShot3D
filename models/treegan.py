...
class TreeGCN(nn.Module):
    def __init__(self, batch, depth, features, degrees, support=10, node=1, upsample=False, activation=True):
        ...
        self.W_root = nn.ModuleList([nn.Linear(
            features[inx], self.out_feature, bias=False) for inx in range(self.depth+1)])
        ...
        self.bias = nn.Parameter(torch.FloatTensor(
            1, self.degree, self.out_feature))
    ...
    def init_param(self):
        if self.upsample:
            init.xavier_uniform_(self.W_branch.data,
    ...
    def forward(self, tree):
        ...
        for inx in range(self.depth+1):
            ...
            root = root + \
                root_node.repeat(1, 1, repeat_num).view(
        ...
        if self.upsample:
            ...
            branch = branch.view(self.batch, self.node *
                                 self.degree, self.in_feature)

            branch = self.W_loop(branch)

            branch = root.repeat(1, 1, self.degree).view(
                self.batch, -1, self.out_feature) + branch
        else:
...
class TreeGAN(nn.Module):
    def __init__(self, batch_size=4, features=[512, 64, 64,  64,  64,  32, 3], degrees=[2,  2,  2,   4,   8,   32], support=10):
        self.batch_size = batch_size
        self.layer_num = len(features)-1
...
class Generator(nn.Module):
    def __init__(self, latent_dim=256, batch_size=4, features=[512, 64, 64,  64,  64,  32, 3], degrees=[2,  2,  2,   4,   8,   32], support=10):
...
class GradientPenalty:
    ...
    def __call__(self, netD, real_data, fake_data):
        ...
        alpha = torch.rand(
            batch_size, 1, 1, requires_grad=True).to(self.device)
