...
from .treegan import Generator, Discriminator, GradientPenalty, TreeGAN, DiscriminatorRaw
import pytorch_lightning as pl
from utils.utils import count_parameters, AverageMeter, plot_image_output_gt, reparameterization, plot_pcd_one_view
...
class BicycleGANModule(pl.LightningModule):
    ...
    def on_train_epoch_start(self):
        self.log('lr', self.scheduler_G.get_last_lr()[0],
    ...
    def training_step(self, batch, batch_idx):
        ...
        for _ in range(self.args.gen_freq):
            ...
            sampled_z = Variable(torch.tensor(np.random.normal(
                0, 1, (real_A.size(0), self.args.latent_dim)), dtype=torch.float32, device=self.device))
            ...
            loss_GE = loss_VAE_GAN + loss_LR_GAN + \
            ...
            loss_latent = self.args.lambda_latent * \
                self.mae_loss(_encoded_z, sampled_z)
        ...
        loss_D_VAE = self.D_VAE.compute_loss(real_B, valid) + self.D_VAE.compute_loss(
            fake_B.detach(), fake) + self.gp_loss(self.D_VAE, real_B.data, fake_B.data)
        ...
        loss_D_LR = self.D_LR.compute_loss(real_B, valid) + self.D_LR.compute_loss(
            _fake_B.detach(), fake) + self.gp_loss(self.D_LR, real_B.data, _fake_B.data)
    ...
    def handle_post_train_step(self, real_A, real_B, fake_B, _fake_B):
        ...
        if self.train_step % self.args.save_iter == 0:
            ...
            plot_image_output_gt(os.path.join(self.dir, 'train', f"train_{self.train_step}.png"),
                                 real_A[index].detach().cpu().transpose(
        ...
        if self.train_step % 100 == 0:
            index = np.random.randint(0, real_A.shape[0])
            wandb.log({"train_B": wandb.Object3D(real_B[index].detach().cpu().numpy()),
                       "train_B_fake": wandb.Object3D(fake_B[index].detach().cpu().numpy()),
                       "train_B_fake_Sampled": wandb.Object3D(_fake_B[index].detach().cpu().numpy()),
                       "train_A": wandb.Image(real_A[index].detach().cpu().transpose(1, 0).transpose(1, 2).numpy())}
                      )
            if self.writer:
                self.writer.add_image(
                    'train_A', real_A[index].detach().cpu().numpy(), self.train_step)
                self.writer.add_mesh('train_B', vertices=real_B[index].detach(
                ).cpu().numpy().reshape(1, -1, 3), global_step=self.train_step)
                self.writer.add_mesh('train_fake_B', vertices=fake_B[index].detach(
                ).cpu().numpy().reshape(1, -1, 3), global_step=self.train_step)
                self.writer.add_mesh('train_fake_B_Sampled', vertices=_fake_B[index].detach(
                ).cpu().numpy().reshape(1, -1, 3), global_step=self.train_step)
                self.writer.flush()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_loss_chamfer', loss_chamfer.item() * 1000, on_step=True,
                 on_epoch=True, prog_bar=True, logger=True)
...
class BicycleGANMuVarModule(BicycleGANModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for _ in range(self.args.gen_freq):
            ...
            loss_GE = loss_VAE_GAN + loss_LR_GAN + self.args.lambda_chamfer * \
                loss_chamfer_dcd + loss_kl * self.args.lambda_kl
            loss_GE *= 10
            loss_GE.backward(retain_graph=True)
            self.optimizer_E.step()
...
class TreeGANRawModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for _ in range(self.args.dis_freq):
            ...
            z = torch.randn(self.args.batch_size, 1,
                            self.args.latent_dim * 2, device=self.device)  # .to(self.device)
            tree = [z]
            with torch.no_grad():
        ...
        self.log_dict(loss_dict)
        self.log('loss', g_loss, on_step=True,
                 on_epoch=True, prog_bar=True, logger=True)
        self.log('lr', self.scheduler_G.get_last_lr()
                 [0], prog_bar=True, logger=False)
    ...
    def handle_post_train_step(self, point, fake):
        ...
        if self.train_step % 500 == 0:
            index = np.random.randint(0, point.shape[0])
            wandb.log({"train_B": wandb.Object3D(point[index].detach().cpu().numpy()),
                       "train_B_fake": wandb.Object3D(fake[index].detach().cpu().numpy())}
                      )
            if self.writer:
                self.writer.add_mesh('train_B', vertices=point[index].detach(
                ).cpu().numpy().reshape(1, -1, 3), global_step=self.train_step)
                self.writer.add_mesh('train_fake_B', vertices=fake[index].detach(
                ).cpu().numpy().reshape(1, -1, 3), global_step=self.train_step)
                self.writer.flush()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        point = B.float()
        z = torch.randn(self.args.batch_size, 1,
                        self.args.latent_dim * 2, device=self.device)
        tree = [z]
        fake = self.generator(tree)
        fake = fake.float()
