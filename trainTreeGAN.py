...
def get_model(args):
    ...
    encoder = PCNEncoder(args.latent_dim)
...
def train(models, trainLoader, valLoader, args):
    ...
    for epoch in range(args.epoch, args.n_epochs):
        ...
        train_loss = 0.0
        for key, value in encoded_priors.items():
        ...
        for i, batch in enumerate(loader):
            ...
            loss_GE = loss_VAE_GAN + loss_LR_GAN + args.lambda_chamfer * loss_chamfer_dcd
            loss_GE *= 10
            ...
            # Latent L1 loss
            _encoded_z = encoder(_fake_B)
            loss_latent = args.lambda_latent * mae_loss(_encoded_z, sampled_z)
            ...
            for i in range(real_A.shape[0]):
                if taxonomy_id[i] in encoded_priors:
                    ...
                    prior_z.append(np.random.normal(
                        0, 1, (args.latent_dim)))
            prior_z = Variable(Tensor(prior_z))
            fake_B = generator(real_A, prior_z)
