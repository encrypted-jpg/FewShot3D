...
from models.memory_decoder import (
    MemoryModule,
    MemoryIAModule,
    BaseImageEncoderModule,
    MHAEncDecoder,
    MHAEncTreeDecoder,
)
import argparse
...
module_dict = {
    ...
    "memoryiamodule": MemoryIAModule,
    "baseimgencoder": BaseImageEncoderModule,
    "mhaencdecoder": MHAEncDecoder,
    "mhaenctreedecoder": MHAEncTreeDecoder,
}
...
def dataLoaders(args, num_workers=24, prefetch_factor=32):
    ...
    trainDataset = PCNImageHDF5Dataset(
        folder,
        json,
        mode="train",
        b_tag=args.b_tag,
    ...
    testDataset = PCNImageHDF5Dataset(
        folder,
        json,
        mode="test",
        b_tag=args.b_tag,
    ...
    valDataset = PCNImageHDF5Dataset(
        folder,
        json,
        mode="val",
        b_tag=args.b_tag,
...
def get_args():
    ...
    parser.add_argument(
        "--json", type=str, default="final.json", help="JSON file containing the data"
    )
    parser.add_argument(
        "--b_tag", type=str, default="depth", help="Tag for the B Image"
    )
    parser.add_argument("--img_count", type=int, default=3, help="Image count")
    parser.add_argument("--module", type=str, default="treegan", help="Module to use")
    ...
    parser.add_argument("--num_dense", type=int, default=8192, help="Number of dense")
    parser.add_argument(
        "--gen_freq", type=int, default=1, help="Generator Training frequency"
    )
    parser.add_argument(
        "--dis_freq", type=int, default=1, help="Discriminator Training frequency"
    )
    ...
    parser.add_argument(
        "--delta", type=float, default=0.05, help="Delta to insert in memory"
    )
    parser.add_argument("--decay_epoch", type=int, default=10, help="Decay epoch")
    parser.add_argument("--save_iter", type=int, default=20, help="Save interval")
    parser.add_argument("--lr", type=float, default=0.0002, help="Learning rate")
    parser.add_argument("--decay_factor", type=float, default=0.5, help="Decay Factor")
    parser.add_argument("--T_0", type=int, default=10, help="T_0")
    parser.add_argument("--gpu", type=int, default=0, help="GPU to use")
    parser.add_argument(
        "--modelPath", type=str, default="bestModel.pth", help="Path to model"
    )
    parser.add_argument("--test", action="store_true", help="Test model")
    parser.add_argument("--testSave", action="store_true", help="Save test output")
    parser.add_argument("--resume", action="store_true", help="Resume training")
    parser.add_argument(
        "--lambda_pixel", type=float, default=10, help="pixelwise loss weight"
    )
    parser.add_argument(
        "--lambda_latent", type=float, default=0.5, help="latent loss weight"
    )
    parser.add_argument(
        "--lambda_kl", type=float, default=0.01, help="kullback-leibler loss weight"
    )
    parser.add_argument(
        "--lambda_chamfer", type=float, default=1, help="chamfer loss weight"
    )
    parser.add_argument(
        "--lambda_snn", type=float, default=0.1, help="snnl loss weight"
    )
...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(
        ...
        log_model="all",
        save_dir=os.path.join(args.log_dir, args.exp),
    )
    ...
    train_loader, test_loader, val_loader = dataLoaders(
        args, num_workers=16, prefetch_factor=32
    )
    args.train_batches = len(train_loader)
    args.val_batches = len(val_loader)
